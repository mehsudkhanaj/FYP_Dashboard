...
import { checkGimminie, submitLogin } from 'app/auth/store/loginSlice';
...
const schema = yup.object().shape({
  // email: yup.string().email('You must enter a valid email').required('You must enter a email'),s
  password: yup
    .string()
    ...
    <div className="w-full">
      <form className="flex flex-col justify-center w-full" onSubmit={handleSubmit(onSubmit)}>
        ...
        <Button
          ...
          value="legacy"
        >
          {loader ? <CircularProgress /> : i18next.t(`navigation:LOGIN`)}
        </Button>
      </form>
    </div>
  );
